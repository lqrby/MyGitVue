...
class ClassTestCase_yzb:
    ...
    def runAllCase(self,app_name,host_key="None"):
        ...
        #获取全部用例
        results = self.loadAllClassByApp(app_name_id = api_host_obj["name_id"])
        for case in results:
            if case["run"] == "yes":
                try:
                    #执行用例
    ...
    def runCase(self,case,api_host_obj):
        ...
        headers["mobile-unid"] = str(int(time.time()))
        headers["mobile-type"] = "ALP-TL00"+str(int(time.time()))
        ...
        if "token" in request_data and request_data.get("token") == "":
