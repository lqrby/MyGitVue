...
from yzb_performance.common.qianMing import GetDataSign
from yzb_performance.common.publicRequestMethod import PublicRequest
from yzb_performance.common.publicData import PublicDataClass
...
class PengYouClass(TaskSet):
        ...
        with self.client.post(login_url, data = login_data, headers = header, verify = False, allow_redirects=False,catch_response=True) as loginres:
            ...
            if "[200]" in str(loginres):
                ...
                if 'status' in loginresult and loginresult["status"] == 200 or 'status' in loginresult and loginresult["status"] == "200":
                    ...
                    loginres.success()
                    time.sleep(random.randint(1,3))
                    login_res = PublicDataClass().miBaoWenTi(apikey,header)
                    return login_res
    ...
    def getUserDetail(self,apikey,header,login_res,uid):
        ...
        xq_data = {
            ...
            "sign":"",
            "timestamp":str(int(time.time())),
            "username":login_res["access_token"]
        }
    ...
    def addBuddy(self,apikey,header,login_res,uid):
        """"
        根据uid添加对方为好友
        """
