...
class PublicDataClass(TaskSet):
    ...
    def login(self,apikey,header):
        ...
        device_tokens = int(time.time() * 1000) #"AkWsVNSPMcwhC6nAXITHbPyrv0YgG5nt1T0B8n79-lrN"
        self.login_data = {
            "password":password,
            "latitude":"39.905662",
            "mobile":str(mobile),
        ...
        print("666login_data===",self.login_data)
        ...
        if 'status' in loginresult and loginresult["status"] == 200 or 'status' in loginresult and loginresult["status"] == "200":
            ...
            print("请打开应用权限")
            loginres.failure("XXX手机号：{}---XXX====密码：{}".format(str(mobile),password))   
    ...
    def miBaoWenTi(self,apikey,header):
        ...
        daan_data ={
            ...
            "device_tokens":str(int(time.time() * 1000)),
            "timestamp":str(int(time.time())),
            "sign":	""
        }
    ...
    def getUserType(self,apikey,header,login_res):
        ...
        #进入获取任务页面请求
        sfzpath = "E:/myTestFile/TestObject/zhongfuan/yunzhangben/yzb_performance/static/shenfenzheng.txt"
