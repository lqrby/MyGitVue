...
class ZFAclassTestCase(TaskSet):
    ...
    def runAllCase(self,caseListId,api_host_obj="None",access_token ="None", last_response = ""):
        ...
        for case in caseArr:
            try:
                #执行用例
                response = self.runCase(case,api_host_obj,last_response)
    ...
    def runCase(self,case,last_response,api_key):
        """
        执行单个用例
        """
        print("runCase")
        ...
        sign = GetDataSign().sign_body(req_url, request_data, api_key) 
        ...
        response = req.requestMethod(req_url, req_urlName, method,headers=headers,param=request_data, content_type=content_type)
        return response
